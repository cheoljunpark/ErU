    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_odom == True and self.is_path == True:
                ...
                min_dis = float('inf')
                current_waypoint = -1
                for idx, node_idx in enumerate(self.global_path_msg.poses):                  
                    global_x = float(node_idx.pose.position.x)
                    global_y = float(node_idx.pose.position.y)
                    distance = sqrt(pow(global_x - x, 2) + pow(global_y - y, 2))
                    if distance < min_dis:  
                        min_dis = distance  
                ...
                if current_waypoint != -1 :
                    if current_waypoint + self.local_path_size < len(self.global_path_msg.poses):
                         for idx in range(current_waypoint, current_waypoint + self.local_path_size):
                            pose = PoseStamped()
                            pose.pose.position.x = self.global_path_msg.poses[idx].pose.position.x
                            pose.pose.position.y = self.global_path_msg.poses[idx].pose.position.y
